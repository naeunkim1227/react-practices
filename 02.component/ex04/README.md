...
1.  state
   ...
   - 상태가 업데이트 되면 컴포넌트의 반응형(Reactive) Rendering이 트리거가 되고 컴포넌트와 자식 컴포넌트가 다시 렌더링 된다.
      ...
      + 컴포넌트 내부의 state을 외부에 정확하게 나타내고 인터페이스의 동기화를 위해서 다시 렌더링한다.
         -기존의 네이티브돔에서 데이터가 변할때 모든 곳에서 변경이 일어나면, 서버에서 부담이 크고 속도가 느리다. 리액트돔에서는 컴포넌트를 연쇄적으로 변동해도(컴포넌트 > 자식 컴포넌트) 부담이 없고, 변동한 부분만 비교해서 sync를 통해 DOM에 반영한다. 

   - 컴포넌트가 동작(event)과 상호작용을 수행할 수 있는 매커니즘을 제공한다.
      + 원래 형식 : 이벤트핸들러를 만들어서 모든 엘리먼트 생성하고, 데이터 추가하는 형식
